\section{\texttt{std::any}}\label{ch17}
...
例如这个例子中，如果你用\texttt{std::string}作为\texttt{std::any\_cast<>}的模板参数，
...
\subsection{\texttt{std::any}类型和操作}
...
\end{itemize}
\texttt{std::any}类也使用了头文件\texttt{<utility>}中的
\texttt{std::in\_place\_type}对象（类型是\texttt{std::in\_place\_type\_t}）。
...
（但也可能有其他的值）：
\begin{lstlisting}
    std::cout << (a.has_value() ? std::any_cast<std::string>(a) :
            std::string("NIL"));
\end{lstlisting}
注意：
\begin{lstlisting}
...
any}和\texttt{std::variant<>}以及\texttt{std::optional<>}的API。Jonathan Wakely在\\
