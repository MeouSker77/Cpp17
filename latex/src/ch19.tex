\section[字符串视图]{字符串视图(String Views)}\label{ch19}
...
\subsubsection{字符串视图有害的一面}\label{ch19.3.1}
...
\subsection{字符串视图类型和操作}
这一节详细描述字符串视图类型和操作

\subsubsection{字符串视图的具体类型}
...
\subsection{在API中使用字符串视图}
字符串视图开销很低并且每一个\texttt{std::string}都可以用作字符串视图。
因此，看起来好像\texttt{std::string\_view}是更好的用作字符串参数的类型。
然而，有一些细节很重要\ldots
...
\end{lstlisting}
注意我们不能简单的用\texttt{operator+}连接两个字符串视图。
作为替代，我们必须把其中一个转换为\texttt{std::string}
...
\subsection{后记}
第一个引用语义的字符串类由Jeffrey Yasskin在\url{https://wg21.link/n3334}中提出
