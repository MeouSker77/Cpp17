...
\subsubsection{\texttt{std::optional<>}操作}
表\nameref{t15.1}列出了\texttt{std::optional<>}的所有操作：\\
\begin{table}[ht]
...
\begin{lstlisting}
    ...
    auto o16 = pos != englishToGerman.end()
                ...
                : std::nullopt;
\end{lstlisting}
这里，根据\nameref{ch9}，依据\texttt{std::optional{pos->second}}表达式能推导出
