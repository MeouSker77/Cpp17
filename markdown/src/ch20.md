...
因此，一个在Windows下的可移植版本应该使用成员函数`string()`，
...
类型`error_code`由C++11引入，它包含了一系列可移植的错误条件，例如
...
| `is_fifo(p)`            | 返回是否文件`p`存在并且是FIFO或者管道文件 |
...
`permissions()`使用了在“权限”小节描述的API。可选的`mode`
...
#### 目录项缓存
鼓励实现 *缓存* 额外的文件属性来避免使用目录项时额外的文件系统访问开销。
